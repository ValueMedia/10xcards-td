...
## 10xDevs AI Toolkit — Module 3 Task Router

Per-lesson full rules in `@.claude/lessons/`. Load the relevant file when working within a specific lesson.

| Lesson | File | Key commands |
|--------|------|-------------|
| L1 — Test strategy & rollout | `@.claude/lessons/m3l1.md` | `/10x-test-plan`, `/10x-test-plan --status`, `/10x-test-plan --refresh` |
| L2 — Writing tests (oracle + TDD) | `@.claude/lessons/m3l2.md` | `/10x-research`, `/10x-plan`, `/10x-implement`, `/10x-tdd`, `m3l2-ad-hoc-testing` |
| L3 — Hooks (per-edit/commit/push) | `@.claude/lessons/m3l3.md` | Hook config in `settings.json`, `lefthook.yml` |
| L4 — E2E tests | `@.claude/lessons/m3l4.md` | `/10x-e2e` skill |

### Lesson boundaries (applies across all lessons)
...
- `context/foundation/test-plan.md` — quality contract (L1); §3 rollout state, §4 quality gates, §6 cookbook
...
- `context/changes/<change-id>/` — per-rollout-phase change folder
- `context/changes/<change-id>/research.md` — oracle source (L2)
- `context/changes/<change-id>/plan.md` — ordered phases with `## Progress` (L2)
- `context/archive/<change-id>/plan.md` — implemented risk surface (L1)
