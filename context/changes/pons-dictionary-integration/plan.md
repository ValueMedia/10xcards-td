...
- [ ] 4.1 `src/pages/api/sets/[id]/generate.ts` — `DICTIONARY_TOOL_DE` declared, `handleToolCall` dispatches both tools, `tools: [DICTIONARY_TOOL, DICTIONARY_TOOL_DE]` passed
- [ ] 4.2 `src/pages/api/sets/[id]/generate.test.ts` — 3 new cases pass (`lookup_word_de` dispatch, unknown-tool fallback, both tools in `tools` array)
- [ ] 4.3 `npx vitest run` full suite green
- [ ] 4.4 `npm run build` succeeds
- [ ] 4.5 `npx eslint src/pages/api/sets/\[id\]/generate.ts` passes
