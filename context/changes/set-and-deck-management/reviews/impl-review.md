...
- **Decision**: FIXED — replaced .single() with .maybeSingle() in getSetWithFlashcards
...
- **Decision**: FIXED — implemented via key={renameTarget?.id} on RenameSetDialog in SetDashboard (React remount pattern, avoids setState-in-effect lint rule)
...
- **Decision**: DISMISSED — sets can be shared publicly (S-07); userId filter would break cross-user read access. RLS is the correct layer for ownership enforcement here.
...
- **Decision**: FIXED — added .select("id") and returns "Set not found" when result.data.length === 0
...
- **Decision**: FIXED — both pages now surface service errors as distinct error states (not mixed with empty/not-found)
...
- **Decision**: SKIPPED — acceptable Astro SSR serialization pattern
...
- **Decision**: FIXED — implemented stretched-link pattern (absolute <a> inset-0 z-0, dropdown z-10)
