...
## Scraper musi sprawdzić `response.ok` przed parsowaniem — non-200 cicho zwraca `[]`

- **Context**: `src/lib/services/dictionary.ts` — `lookupWord` (testing-external-integrations, Phase 1). Cambridge Dictionary scrapowany przez `HTMLRewriter`.
- **Problem**: `lookupWord` nigdy nie sprawdzał `response.ok`. Body strony błędu (503/500) trafiało do `HTMLRewriter`, nie pasowało do żadnego selektora i zwracało `[]`. Na granicy API „słownik padł" było nieodróżnialne od „nieznane słowo" — oba dawały `200 {entries:[]}`. Endpointowy catch 502 odpalał się tylko na twardym odrzuceniu `fetch`, nie na non-200.
- **Rule**: Każdy scraper/parser odpowiedzi HTTP musi sprawdzić `response.ok` (lub status) PRZED parsowaniem i rzucić na non-200, żeby caller mógł zwrócić czysty błąd (np. 502) zamiast cicho pustego wyniku. Uwaga na kolejność: jeśli istnieje short-circuit na redirect (np. nieznane słowo 302→200 na base URL), zostaw go PRZED sprawdzeniem `response.ok`, żeby prawidłowy-ale-nieznany przypadek nadal zwracał `[]`, a nie rzucał.
- **Applies to**: wszystkie serwisy scrapujące/parsujące odpowiedzi zewnętrznych integracji (dictionary, AI provider, dowolny upstream HTTP)

## Test route'u API importującego `astro:env/server` w projekcie `node`: alias do stubu + mock rate-limitu

- **Context**: `src/pages/api/sets/[id]/generate.test.ts` (testing-external-integrations, Phase 3). Trzy projekty Vitest: `node`, `workers`, `integration`.
- **Problem**: Projekt `node` nie aliasował `astro:env/server`, więc route generujący (który go importuje przez `generate.ts:14`) nie dawał się nawet zaimportować pod `node`. Dodatkowo gate rate-limit fail-closed: pod stubem `env={}` → `kv` null → `checkRateLimit` zwraca `{allowed:false}`, więc każdy request kończył się 429 zanim dotarł do właściwej logiki.
- **Rule**: Aby testować route API importujący `astro:env/server` w projekcie `node`, dodaj alias `"astro:env/server" → src/test/astro-env-server.stub.ts` w `resolve.alias` projektu `node` (wzór: projekt `integration`, `vitest.config.ts`). Stub czyta `process.env`, więc gałąź configured/unconfigured sterujesz przez `process.env.OPENROUTER_API_KEY`. Zawsze `vi.mock` `checkRateLimit` z domyślą `{allowed:true}` — inaczej fail-closed na null-KV zamienia każdy request w 429. Gdy testujesz mapowanie error→status, użyj **partial-mocka** (`{ ...(await orig()), generateFlashcardProposals: vi.fn() }`), żeby prawdziwe `getAiErrorHttpStatus` nadal działało i test miał zęby.
- **Applies to**: testy route'ów API w projekcie `node` Vitest, które importują `astro:env/server` lub przechodzą przez gate rate-limit
